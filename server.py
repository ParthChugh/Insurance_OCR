...
def api1text():
    ...
    with open(base_path+filename.split("_")[0]+".csv", 'w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(["INSURED", flag_insured])
        writer.writerow(["PRODUCER", flag_producer])
        writer.writerow(["EMAIL", flag_contact_email])
        writer.writerow(["NAME", flag_contact_name])
        writer.writerow(["DATE", flag_date])
        for i in flag_insurer:
            writer.writerow(["INSURER", i])
...
def api2text():
    ...
    with open(base_path+filename.split("_")[0]+".csv", 'a', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(["CERTIFICATE HOLDER", flag_certificate ])   
    ...
    with open(base_path+filename.split("_")[0]+".csv", 'a', newline='') as file:
        ...
        for k in range(0,len(flag_pollution_value)):
            if(not str(flag_pollution_value[k])==""):
                ...
                result+=(" &#13 ")
                writer.writerow(["", flag_pollution_value[k]])
        result+=(" &#13 ")
